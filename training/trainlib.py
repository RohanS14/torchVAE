...
def trainCPCVAE(cpcvae, unlabeled_data_loader, labeled_data_loader, epochs=20, lr=0.001, beta=1, \
    ...
    for epoch in progress:
        ...
        for i in range(num_batches):
            ...
            # process unlabeled data
            mu_u, logvar_u, probs_xhat_u, x_hat_u, logits_z_u, logits_zhat_u = cpcvae(x_u)
            
            assert x_u.shape == x_hat_u.shape, f"Shapes don't match: {x_u.shape} and {x_hat_u.shape}"
            ...
            # TODO: check if logits/probs are right
            consistency_loss_u = criterion_consistency(logits_zhat_u, F.softmax(logits_z_u, dim=-1))
            loss_u += lambda_ * consistency_loss_u
            ...
            consistency_loss_l = criterion_consistency(logits_zhat_l, F.softmax(logits_z_l, dim=-1))
            
            # aggregate label consistency
            # cross entropy loss between the distribution of predicted labels and some target (uniform or the distribution of true labels)
            probs = F.softmax(logits_z_u, dim=-1)
            preds = torch.argmax(probs, dim=-1)
            counts = torch.bincount(preds, minlength=cpcvae.classifier.num_classes)
            assert counts.shape == (cpcvae.classifier.num_classes,), f"Counts shape is wrong: {counts.shape}"
            counts = counts / counts.sum()
            assert counts.sum() == 1, f"Counts don't sum to 1: {counts.sum()}"
            agg_loss = criterion_aggregate(counts, torch.ones_like(counts) / cpcvae.classifier.num_classes)
            
            loss_l = recon_loss_l + beta * kl_loss_l + lambda_ * class_loss + lambda_ * consistency_loss_l + agg_loss
        ...
        with torch.no_grad():
            # TODO: log scaled or unscaled losses?
            recon_loss = u_weight * recon_loss_u + l_weight * recon_loss_l
            kl_loss = beta * (u_weight * kl_loss_u +l_weight * kl_loss_l)
            class_loss = lambda_ * l_weight * class_loss
            consistency_loss = lambda_ * (u_weight * consistency_loss_u + l_weight * consistency_loss_l)
        ...
        progress.set_description(f'Total Loss: {total_loss:.3f}, Recon: {recon_loss:.3f}, KL: {kl_loss:.3f}, Class: {class_loss:.3f},' 
